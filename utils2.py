...
BATCH_SIZE = 8
FIGURE_FOLDER = '/home/mila/c/chris.emezue/representation-learning-assignment/figs/model-downscaled/default/'
...
def random_crop_augmentation(x_train_tensor):
  #while (r2 - r1) * torch.rand(a, b) + r1 produces numbers in the range [r1, r2).

  #gaussian_noise = 2 * torch.rand(x_train_tensor.shape) -1
  zero_ones = torch.bernoulli(torch.empty(x_train_tensor.shape).uniform_(0, 1))
  augmented = x_train_tensor * zero_ones
  #gaussian_noise_filtered = gaussian_noise*zero_ones
  #gaussian_data = x_train_tensor + gaussian_noise_filtered
  return augmented

def gaussian_noise_augmentation(x_train_tensor):
  #while (r2 - r1) * torch.rand(a, b) + r1 produces numbers in the range [r1, r2).
