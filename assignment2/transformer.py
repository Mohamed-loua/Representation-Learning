...
class LayerNorm(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # ==========================
        expectation_input = torch.mean(inputs,dim=-1,keepdim=True)
        variance_input = torch.var(inputs,dim=-1,keepdim=True,correction=0)

        scaled_x = torch.div(inputs - expectation_input,torch.sqrt(variance_input + self.eps))
        layered_x = scaled_x*self.weight + self.bias
        return layered_x
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, head_size, num_heads, sequence_length=0):
        ...
        self.W_Q = nn.Parameter(torch.empty(self.head_size*self.num_heads,self.head_size*self.num_heads))
        self.b_Q = nn.Parameter(torch.empty(self.head_size*self.num_heads))

        self.W_K = nn.Parameter(torch.empty(self.head_size*self.num_heads,self.head_size*self.num_heads))
        self.b_K = nn.Parameter(torch.empty(self.head_size*self.num_heads))

        self.W_V = nn.Parameter(torch.empty(self.head_size*self.num_heads,self.head_size*self.num_heads))
        self.b_V = nn.Parameter(torch.empty(self.head_size*self.num_heads))
        
        self.W_output = nn.Parameter(torch.empty(self.head_size*self.num_heads,self.head_size*self.num_heads))
        self.b_output = nn.Parameter(torch.empty(self.head_size*self.num_heads))
    ...
    def get_attention_weights(self, queries, keys, mask=None):
        ...
        # Let's start with the naive method
        if mask is not None:
            mask = mask.unsqueeze(-2)
            #.expand(-1,-1,self.sequence_length)
        
        head_attns = []
        for i in range(self.num_heads):
            q_i = queries[:,i,:,:]
            k_i = keys[:,i,:,:]
            #k_i_ = k_i.permute((0,2,1))
            k_i= k_i.transpose(-2,-1)
            aa_i = q_i @ k_i
            aa_i = aa_i / np.sqrt(self.head_size)
            if mask is not None:
                #aa_i = aa_i * mask
                aa_i = aa_i.masked_fill(mask==0,float('-inf'))
  
            # need to do softmax
            output = F.softmax(aa_i,dim=-1)            
            head_attns.append(output)

        concat_head_attn = torch.stack(head_attns,1)
        return concat_head_attn
    ...
    def split_heads(self, tensor):
        ...
        # TODO: Write your code here
        # ==========================
        bs,seq_len,conc_heads = tensor.size()
        tensor = tensor.reshape((bs,seq_len,self.num_heads,int(conc_heads/self.num_heads)))
    ...
    def forward(self, hidden_states, mask=None):
        ...
        # TODO: Write your code here
        # ==========================
        Q = hidden_states @ self.W_Q.transpose(-2,-1) + self.b_Q
        K = hidden_states @ self.W_K.transpose(-2,-1) + self.b_K
        V = hidden_states @ self.W_V.transpose(-2,-1) + self.b_V
        ...
        attn_output = self.apply_attention(Q,K,V,mask)
        
        output = attn_output @ self.W_output.transpose(-2,-1) + self.b_output
        return output
...
class PreNormAttentionBlock(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        layered_x = self.layer_norm_2(attention_outputs)
        layered_x = self.linear(layered_x)
        outputs = layered_x + attention_outputs
        return outputs
...
class Transformer(nn.Module):
    ...
    def __init__(self, vocabulary_size=30522, embed_dim=256, hidden_dim=256, num_heads=1,
        ...
        #self.cls_mask = nn.Parameter(torch.full(fill_value=1,size=(1,1)),requires_grad=False)
    ...
    def forward(self, x, mask=None):
        ...
        # ==========================
        # if mask is not None:
        #     #cls_mask =torch.full(fill_value=1,size=(B,1))
        #     cls_mask = self.cls_mask.expand((B,-1))
