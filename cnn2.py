...
FIGURE_FOLDER = os.path.join(FIGURE_FOLDER,f'cnn')
...
#x = train_dataset[0][0].reshape(3,80)
#pad = transforms.Pad(50, fill=0, padding_mode='constant')
#random_erase = transforms.RandomErasing(p=1)

# breakpoint()
# fig, ax = plt.subplots
# g0 = gaussian_data[0].reshape((3,80))[0]
# g1 = x_train_tensor[0].reshape((3,80))[0]
# ax.plot(gaussian_data[0].reshape((3,80))[0])
# ax.plot(g0)
# fig.savefig('random_crop_x.png')

# #gaussian_data = gaussian_noise_augmentation(x_train_tensor)
# percent = 25
# size_to_use = int(percent / 100 * len(x_train_tensor))
# #breakpoint()
# gaussian_data = random_crop_augmentation(x_train_tensor)

# x_train_tensor = torch.cat([x_train_tensor,gaussian_data[:size_to_use]],0)
# y_train_tensor = torch.cat([y_train_tensor,y_train_tensor[:size_to_use]],0)
